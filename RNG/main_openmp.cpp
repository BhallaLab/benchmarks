...
int main(int argc, char *argv[])
{
    ...
    unsigned int numThreads = 32;                /* No of threads to launch */
...
//        printf("[INFO] Created stl RNG : %p, seed = %d\n" , &rngVec[i], seed*i );
     ...
     *  a container.
     -------------------------------------------------------------*/
    nn = 20;
    vector<long> result(N);                     /* store generated nums */
    ...
    for (int i = 0; i < nn; i++) {
#if USE_GSL
        generate_rand_vec<gsl_rng_type*>( rngVec[0], result, timeTaken);
#else
        generate_rand_vec<mt19937>( rngVec[0], result, timeTaken);
#endif
...
#if USE_GSL
        generate_rand_vec<gsl_rng_type*>( rngVec[tid], result, threadTime);
#else
        generate_rand_vec<mt19937>( rngVec[tid], result, threadTime);
#endif
