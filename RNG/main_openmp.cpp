...
template<typename T>
void generate_random_nums( T rng, long int& sum, double& time )
{
...
int main(int argc, char *argv[])
{
    ...
    ofstream runTimeInfo;
    runTimeInfo.open( "benchmarkdata.csv", ios::app );
    runTimeInfo << "#RNG,Number of samples, serial mean, serial std"
        << ",Number of threads,threaded mean, threaded std"
        << ",total clock time,\n";

    unsigned int seed = 2;                      /* Seed for RNG. */

    unsigned int numThreads;
    /* No of threads to launch can be modified by changing the environment
     * variable OPENMP_NUM_THREADS 
     */
    const char* numThreadEnv = getenv("OPENMP_NUM_THREADS"); 
    if( NULL != numThreadEnv )
        numThreads = atoi( numThreadEnv );
    else
        numThreads = 2;

    cerr << "[INFO] Using " << numThreads << " threads. " << endl;
    ...
    runTimeInfo << "STL,";
#endif 
    runTimeInfo << NUMBERS_TO_GENERATE << ",";
...
//        printf("[INFO] Created stl RNG : %p, seed = %d\n" , &rngVec[i], seed*i );
    ...
    runTimeInfo <<  get<0>(serialRes) << "," <<  get<1>(serialRes) << ",";

    omp_set_num_threads(numThreads);
    runTimeInfo << numThreads << ",";
...
//        printf( "\t%d thread, Time taken %f\n", tid, threadTime);
    ...
    runTimeInfo << (t2 - t1) * 1000;
    runTimeInfo << endl;
    runTimeInfo.close();
    cout << "Appended to benchmarkdata.csv file" << endl;
    return 0;
}
