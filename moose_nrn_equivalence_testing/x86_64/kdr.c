...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 10, 4);
 	...
 	ivoc_help("help ?1 kdr /home_local/dilawars/Work/BhallaLab/benchmarks/moose_nrn_equivalence_testing/x86_64/kdr.mod\n");
...
static int _reset;
static char *modelname = "Channel: kdr";
...
/*CVODE*/
 ...
 static double _mfac_rates, _tmin_rates;
  ...
  if (_maktable) { double _x, _dx; _maktable=0;
   _tmin_rates =  - 100.0 ;
   _tmax =  100.0 ;
   _dx = (_tmax - _tmin_rates)/2000.; _mfac_rates = 1./_dx;
   for (_i=0, _x=_tmin_rates; _i < 2001; _x += _dx, _i++) {
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
   double _lalpha , _lbeta , _ltau , _linf , _lgamma , _lzeta , _ltemp_adj_n , _lA_alpha_n , _lB_alpha_n , _lVhalf_alpha_n , _lA_beta_n , _lB_beta_n , _lVhalf_beta_n , _lA_tau_n , _lB_tau_n , _lVhalf_tau_n , _lA_inf_n , _lB_inf_n , _lVhalf_inf_n ;
  _ltemp_adj_n = 1.0 ;
   _lalpha = ( exp ( ( 1e-3 * - 3.0 * ( _lv - 13.0 ) * 9.648e4 ) / ( 8.315 * ( 273.16 + ( celsius ) ) ) ) ) ;
   _lbeta = ( exp ( ( 1e-3 * - 3.0 * 0.7 * ( _lv - 13.0 ) * 9.648e4 ) / ( 8.315 * ( 273.16 + ( celsius ) ) ) ) ) ;
   if ( _lbeta / ( 0.02 * ( 1.0 + _lalpha ) ) < 2.0 ) {
     _ltau = 2.0 ;
     }
   else {
     _ltau = _lbeta / ( 0.02 * ( 1.0 + _lalpha ) ) ;
     }
   ntau = _ltau / _ltemp_adj_n ;
   _linf = 1.0 / ( 1.0 + _lalpha ) ;
   ninf = _linf ;
    return 0; }
