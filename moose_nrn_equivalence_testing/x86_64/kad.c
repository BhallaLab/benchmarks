...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 8, 3);
 	...
 	ivoc_help("help ?1 kad /home_local/dilawars/Work/BhallaLab/benchmarks/moose_nrn_equivalence_testing/x86_64/kad.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "K-A Distal channel";
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 58 in file kad.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
